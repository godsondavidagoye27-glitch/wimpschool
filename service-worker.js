...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache =>
      // cache.addAll() fails (and skips caching everything) if a single asset
      // 404s or is missing (e.g. js/config.js before it has been generated).
      // Cache each asset independently so one missing file doesn't take down
      // offline support for the rest of the app.
      Promise.all(
        ASSETS.map(asset =>
          cache.add(asset).catch(err => console.warn('Service worker: skipping uncachable asset', asset, err))
        )
      )
    )
  );
