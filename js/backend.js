...
async function inviteTeacher(payload) {
  if (!supabase?.functions?.invoke) {
    return { error: { message: 'Invite service is not available.' } };
  }
  ...
  try {
    ...
    return { error: { message: err?.message || 'Unable to create teacher invite.' } };
  }
}
...
async function inviteParent(payload) {
  ...
  try {
    ...
    return { error: { message: err?.message || 'Unable to create parent invite.' } };
  }
}
...
async function recordPayment(payload) {
  if (!supabase?.functions?.invoke) {
    return { error: { message: 'Supabase Functions are not available. Cannot record payment securely.' } };
  }
  ...
  try {
    ...
    const response = await supabase.functions.invoke('payments', {
    ...
    if (data) {
      await sendSchoolNotification({
        schoolId: payload.schoolId,
        type: 'payment',
        message: `Payment received for ₦${Number(payload.amount || 0)}`
      });
    }

    return { data, error: null };
  } catch (err) {
    return { error: { message: err?.message || 'Unable to record payment.' } };
  }
}
...
async function sendSchoolNotification(payload) {
  if (!supabase?.functions?.invoke) {
    return { error: { message: 'Supabase Functions are not available. Cannot send notifications securely.' } };
  }
  ...
  try {
    ...
    return { error: { message: err?.message || 'Unable to send notification via backend.' } };
  }
}
