...
async function ensureUserRole(userId, role = 'school_admin', schoolId = null) {
  ...
  if (!client.functions || typeof client.functions.invoke !== 'function') {
    return { error: { message: 'Supabase Functions are not available. Cannot ensure user role.' } };
  }
  ...
  try {
    ...
    if (signupData.session) {
      ...
      if (!signInResult.error && signInResult.data?.session) {
        await client.auth.setSession(signInResult.data.session);
      }
    }
    ...
    const ensureResult = await ensureUserRole(userId, 'school_admin', schoolId);
...
async function acceptInvite(token, password) {
  ...
  if (signUpResult.data?.session) {
    await client.auth.setSession(signUpResult.data.session);
  }
  ...
  const ensureResult = await ensureUserRole(userId, invite.role, invite.school_id || null);
